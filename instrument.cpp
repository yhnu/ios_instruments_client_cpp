...
static void add_filter(const char* filterstr)
{
	...
	while (p <= end) {
		if ((*p == '|') || (*p == '\0')) {
			if (p-start > 0) {
				char* procn = (char*)malloc(p-start+1);
				if (!procn) {
				...
				if (!endp || *endp == 0) {
					int *new_pid_filters = (int*)realloc(pid_filters, sizeof(int) * (num_pid_filters+1));
					if (!new_pid_filters) {
					...
					char **new_proc_filters = (char**)realloc(proc_filters, sizeof(char*) * (num_proc_filters+1));
					if (!new_proc_filters) {
						fprintf(stderr, "ERROR: realloc() failed\n");
						exit(EXIT_FAILURE);
					}
...
static void syslog_callback(char c, void *user_data)
{
	...
	if (lp >= line_buffer_size-1) {
		...
		char* _line = (char*)realloc(line, line_buffer_size);
		if (!_line) {
			fprintf(stderr, "ERROR: realloc failed\n");
			exit(EXIT_FAILURE);
		}
	...
	while ((c = getopt_long(argc, argv, "dhu:nxt:T:m:e:p:qkKv", longopts, NULL)) != -1) {
		...
		case 'm':
			if (!*optarg) {
				...
				char **new_msg_filters = (char**)realloc(msg_filters, sizeof(char*) * (num_msg_filters+1));
				if (!new_msg_filters) {
					fprintf(stderr, "ERROR: realloc() failed\n");
					exit(EXIT_FAILURE);
				}
		...
		case 't':
			if (!*optarg) {
				...
				char **new_trigger_filters = (char**)realloc(trigger_filters, sizeof(char*) * (num_trigger_filters+1));
		...
		case 'T':
			if (!*optarg) {
				...
				char **new_untrigger_filters = (char**)realloc(untrigger_filters, sizeof(char*) * (num_untrigger_filters+1));
